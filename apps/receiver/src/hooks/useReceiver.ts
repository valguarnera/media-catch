...
import type { ReceiverState, ConnectionState, ReceiverEvent, Capability } from '../types.js';

const INITIAL_STATE: ReceiverState = {
...
export function useReceiver() {
  const [state, setState] = useState<ReceiverState>(INITIAL_STATE);
  const signalingRef = useRef<SignalingService | null>(null);
  const webrtcRef = useRef<WebRTCService | null>(null);
  const initializedRef = useRef(false);

  const updateState = useCallback((partial: Partial<ReceiverState>) => {
  ...
  const handleSignalingEvent = async (event: ReceiverEvent, webrtc: WebRTCService) => {
    switch (event.type) {
      case 'state-changed':
      ...
      case 'answer-received':
        if (webrtcRef.current) await webrtcRef.current.setRemoteAnswer(event.answer.payload.sdp);
        break;
      case 'ice-candidate-received':
        if (webrtcRef.current) await webrtcRef.current.addIceCandidate(
          event.candidate.payload.candidate,
          event.candidate.payload.sdpMid,
          event.candidate.payload.sdpMLineIndex
        );
        break;
      case 'error':
  ...
  const handleWebRTCEvent = (event: ReceiverEvent) => {
    switch (event.type) {
      case 'ice-candidate':
  ...
  const handleOffer = async (
    offerMessage: { payload: { sdp: string; streams: unknown[] } },
