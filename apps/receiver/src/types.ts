...
export type ReceiverEvent =
  | { type: 'state-changed'; state: Partial<ReceiverState> }
  | { type: 'session-code'; code: string }
  | { type: 'peer-connected' }
  | { type: 'peer-disconnected' }
  | { type: 'track-received'; stream: MediaStream }
  | { type: 'track-ended'; kind: 'audio' | 'video' }
  | { type: 'error'; message: string }
  | { type: 'ice-connection-state-changed'; state: RTCPeerConnectionState }
  | { type: 'offer-received'; offer: OfferMessage }
  | { type: 'answer-received'; answer: AnswerMessage }
  | { type: 'ice-candidate-received'; candidate: IceCandidateMessage }
